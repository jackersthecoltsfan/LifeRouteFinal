...
- Checked-in `LifeRoute/`, `LifeRouteLiveActivityWidget/`, and `LifeRoute.xcodeproj/` are the canonical v0.9.1 shipping source. The source/application version is separate from the current canonical Git SHA and any external TestFlight build number; verify those identities independently. Never reconstruct current development by replaying archived v0.x patches.
