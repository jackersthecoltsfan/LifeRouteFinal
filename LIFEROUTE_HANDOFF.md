...
- **STOP_VALIDATION_FAILED:** `scripts/validate_full.sh` reaches a pre-existing timer assertion in `scripts/visual_timer_presentation_tests.swift:229`: `one modal presentation owner`, requiring one `.fullScreenCover(`. Accepted Timer D uses its root-hosted hero instead. The unchanged consolidated donor reproduces the same SIGTRAP under Xcode 27; all involved timer/validator bytes match. No timer source/test correction or gate waiver was made. Native integration, app build, signing and phone install remain NOT RUN. Preserve this boundary for an explicit disposition of the stale validator.
