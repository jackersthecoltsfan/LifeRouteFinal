...
struct VisualAIAssistedStudioView: View {
    ...
    private var iconAICard: some View {
        VStack(alignment: .leading, spacing: 13) {
            HStack {
                VStack(alignment: .leading, spacing: 2) {
                    Text("ABA Icon Creator")
                        .font(.title3.weight(.bold))
                        .foregroundStyle(palette.textPrimary)
                    Text("Interpret the concept first, then create a clear visual that can be saved into the existing icon library.")
                        .font(.caption)
                        .foregroundStyle(palette.textSecondary)
                }
                Spacer()
            ...
            if !imagePlaygroundAvailable {
                ...
                Text("Image generation stays inside Apple’s Image Playground experience. LifeRoute interprets the requested concept first so the resulting artwork is functional for ABA and PECS-style supports.")
                    .font(.caption)
                    .foregroundStyle(palette.textSecondary)
            }
        }
