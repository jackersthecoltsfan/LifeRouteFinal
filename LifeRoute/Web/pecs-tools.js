// LifeRoute PECS-style visual tools: camera icon maker, visual First/Then, and choice boards.
// Images stay on-device/in-browser in localStorage. No upload service is used.
(() => {
  const STORE = "liferoute_pecs_tools_v1";
  const MAX_ICONS = 18;
  let state = {
    icons: [],
    firstIconId: "",
  ...
  const ensureTools = () => {
    ...
    if (!view || !grid || document.getElementById("pecsIconTool")) return false;

    const iconTool = document.createElement("div");
    iconTool.className = "card toolCard";
    iconTool.id = "pecsIconTool";
    iconTool.innerHTML = `
      <div class="toolHead"><div class="toolIcon">◫</div><div class="grow"><div class="title">PECS icon maker</div><div class="meta">Take a photo, add a label, and LifeRoute turns it into your clean PECS-style card.</div></div></div>
      <div class="pecsCaptureLayout">
        <button class="pecsCameraTile" id="pecsCameraButton" type="button">
          <div class="pecsCameraGlyph">⌁</div>
          <b>Use camera</b>
          <span>or choose a photo</span>
        </button>
        <div class="pecsDraftCard">
          <img id="pecsDraftImage" alt="" hidden>
          <div id="pecsDraftEmpty"><b>No photo yet</b><span>Photograph the object, activity, person, or place you want represented.</span></div>
        </div>
      </div>
      <input id="pecsCameraInput" type="file" accept="image/*" capture="environment" hidden>
      <div class="toolInline pecsLabelRow">
        <input id="pecsIconLabel" maxlength="28" placeholder="Label · e.g. Outside">
        <button class="goldButton" id="createPecsIcon" type="button">Create icon</button>
      </div>
      <div class="tiny">Style: white card, bold graphic edges, bright high-contrast color, centered square image, and a clear sans-serif label. Photos are processed locally and are not uploaded.</div>
      <div class="pecsLibraryHead"><b>Icon library</b><span id="pecsIconCount" class="tiny">0 icons</span></div>
      <div id="pecsIconLibrary" class="pecsIconLibrary"></div>
    `;
    ...
    boardTool.innerHTML = `
      <div class="toolHead"><div class="toolIcon">▦</div><div class="grow"><div class="title">Choice board creator</div><div class="meta">Build a visual board from your saved PECS icons. Default layout matches an 8-choice, 2 × 4 board.</div></div></div>
      <div class="grid2">
      ...
      <div class="pecsLibraryHead"><b>Choose icons</b><span id="choiceBoardCount" class="tiny">0 / 8 selected</span></div>
    ...
    if (firstThen) {
      firstThen.insertAdjacentHTML("beforeend", `
        <div class="pecsFirstThenControls">
          <div class="row"><div><b>Visual option</b><div class="tiny">Add PECS icons above the words.</div></div><label class="switch"><input id="firstThenVisualToggle" type="checkbox" checked><span class="slider"></span></label></div>
          <div class="grid2 pecsFirstThenSelects">
          ...
          <div class="tiny">Create or photograph new visuals in PECS icon maker, then select them here.</div>
        </div>
      `);
    }
  ...
  styles.id = "pecsToolStyles";
  styles.textContent = `
    .pecsCaptureLayout{display:grid;grid-template-columns:150px 1fr;gap:9px;margin-bottom:9px}.pecsCameraTile{min-height:145px;border:1px dashed color-mix(in srgb,var(--blue) 44%,var(--line));background:color-mix(in srgb,var(--blue) 5%,var(--panel2));color:var(--text);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px}.pecsCameraTile span{font-size:9px;color:var(--muted)}.pecsCameraGlyph{font-size:28px;color:var(--blue);margin-bottom:4px}.pecsDraftCard{min-height:145px;border-radius:14px;border:1px solid var(--line);background:#fff;overflow:hidden;display:grid;place-items:center}.pecsDraftCard img{width:100%;height:145px;object-fit:cover}.pecsDraftCard #pecsDraftEmpty{color:#243348;text-align:center;padding:18px;display:grid;gap:4px}.pecsDraftCard #pecsDraftEmpty span{font-size:9px;color:#66768b;line-height:1.4}.pecsLabelRow{margin-top:8px}.pecsLibraryHead{display:flex;justify-content:space-between;align-items:center;margin:12px 0 7px}.pecsIconLibrary,.choiceBoardPicker{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:7px}.pecsLibraryItem,.choicePick{position:relative;border:1px solid var(--line);border-radius:12px;padding:6px;background:color-mix(in srgb,var(--panel2) 76%,transparent);text-align:center;color:var(--text);overflow:hidden}.pecsLibraryItem img,.choicePick img{width:100%;aspect-ratio:1;object-fit:cover;border-radius:8px;background:white}.pecsLibraryItem b,.choicePick b{display:block;font-size:9px;margin-top:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.pecsDelete{position:absolute;right:3px;top:3px;width:24px;height:24px;padding:0;border-radius:999px;background:rgba(8,15,26,.82);color:white;font-size:13px}.choicePick.selected{border-color:var(--gold);box-shadow:inset 0 0 0 1px var(--gold);background:color-mix(in srgb,var(--gold) 8%,var(--panel2))}.choicePick.selected:after{content:"✓";position:absolute;top:5px;right:5px;width:21px;height:21px;border-radius:999px;display:grid;place-items:center;background:var(--gold);color:var(--bg);font-size:11px;font-weight:950}.pecsFirstThenControls{margin-top:10px;padding-top:10px;border-top:1px solid var(--line)}.pecsFirstThenSelects{margin-top:8px}
    ...
    @media(max-width:680px){.pecsCaptureLayout{grid-template-columns:115px 1fr}.pecsCameraTile,.pecsDraftCard{min-height:118px}.pecsDraftCard img{height:118px}.pecsIconLibrary,.choiceBoardPicker{grid-template-columns:repeat(3,minmax(0,1fr))}.choiceBoardCell{min-height:150px;border-width:2px;border-radius:16px;padding:8px}.firstThenVisualImage{width:min(70vw,300px);max-height:27vh;margin-bottom:10px}}
  `;
  ...
  const renderLibraries = () => {
    ...
    const lib = document.getElementById("pecsIconLibrary");
    const picker = document.getElementById("choiceBoardPicker");
    const count = document.getElementById("pecsIconCount");
    ...
    if (lib) {
      lib.innerHTML = state.icons.length ? state.icons.map(item => `
        <div class="pecsLibraryItem">
          <img src="${item.dataURL}" alt="${safe(item.label)}">
          <b>${safe(item.label)}</b>
          <button class="pecsDelete" type="button" data-delete-pecs="${safe(item.id)}" aria-label="Delete icon">×</button>
        </div>
      `).join("") : '<div class="tiny" style="grid-column:1/-1;padding:8px 0">No icons yet. Use the camera above to make your first one.</div>';
      lib.querySelectorAll("[data-delete-pecs]").forEach(button => {
        button.onclick = () => {
          const id = button.dataset.deletePecs;
    ...
    if (picker) {
      picker.innerHTML = state.icons.length ? state.icons.map(item => `
        <button class="choicePick ${state.boardSelection.includes(item.id) ? "selected" : ""}" type="button" data-choice-pecs="${safe(item.id)}">
          <img src="${item.dataURL}" alt="${safe(item.label)}"><b>${safe(item.label)}</b>
        </button>
      `).join("") : '<div class="tiny" style="grid-column:1/-1;padding:8px 0">Create icons first, then choose them for the board.</div>';
      picker.querySelectorAll("[data-choice-pecs]").forEach(button => {
        button.onclick = () => {
          const id = button.dataset.choicePecs;
  ...
  const makePecsIcon = async (file, label) => {
  ...
  const chooseDraft = file => {
    ...
    const img = document.getElementById("pecsDraftImage");
    const empty = document.getElementById("pecsDraftEmpty");
    if (img) {
      img.src = draftURL;
      img.hidden = false;
    }
  ...
  const createIcon = async () => {
    const label = String(document.getElementById("pecsIconLabel")?.value || "").trim();
    if (!draftFile) {
    ...
    const button = document.getElementById("createPecsIcon");
    if (button) {
      button.disabled = true;
      button.textContent = "Creating…";
    }
    try {
      const dataURL = await makePecsIcon(draftFile, label);
      const item = {
        id: `pecs-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
        label,
        dataURL,
      ...
      document.getElementById("pecsIconLabel").value = "";
      draftFile = null;
      const img = document.getElementById("pecsDraftImage");
      const empty = document.getElementById("pecsDraftEmpty");
      if (img) {
      ...
      renderAllPecs();
      if (typeof setStatus === "function") setStatus(`${label} PECS icon created`);
    } catch (_) {
      alert("LifeRoute could not process that photo. Try another image.");
    } finally {
      if (button) {
        button.disabled = false;
        button.textContent = "Create icon";
      }
    }
  };
  ...
  const bind = () => {
    const camera = document.getElementById("pecsCameraInput");
    document.getElementById("pecsCameraButton")?.addEventListener("click", () => camera?.click());
    camera?.addEventListener("change", () => chooseDraft(camera.files?.[0]));
    document.getElementById("createPecsIcon")?.addEventListener("click", createIcon);
