...
(() => {
  ...
  style.textContent = `
    ...
    #lifeRouteSettingsOverlay .lrThemeAccordionBody{display:grid;grid-template-rows:0fr;transition:grid-template-rows .22s cubic-bezier(.16,1,.3,1),opacity .16s ease;opacity:.35}
    ...
    #lifeRouteSettingsOverlay .lrThemeAccordion .lrThemeCard{min-height:86px!important;border-radius:15px!important;padding:10px!important;transition:transform .14s cubic-bezier(.2,.8,.2,1),border-color .16s ease,box-shadow .16s ease!important}
    #lifeRouteSettingsOverlay .lrThemeAccordion .lrThemeCard:active{transform:scale(.98)}
    ...
    @media(max-width:680px){#lifeRouteSettingsOverlay .lrSettingsSection.lrThemeAccordion{backdrop-filter:none!important;-webkit-backdrop-filter:none!important}}
  ...
  const bindObserver = () => {
    ...
    if (!overlay || overlay === observedOverlay) return !!overlay;
    observer?.disconnect();
    observedOverlay = overlay;
    observer = new MutationObserver(queueSync);
    observer.observe(overlay,{subtree:true,childList:true,attributes:true,attributeFilter:['class','aria-pressed']});
    queueSync();
    return true;
  };
  const start=()=>{
    ...
    document.addEventListener('click',event=>{
      const target=event.target?.closest?.('[data-view="settings"],#openSettings,.lrSettingsButton,[data-open-settings]');
      if(target) window.setTimeout(()=>{bindObserver();queueSync();},0);
    },true);
    [180,600,1400].forEach(ms=>window.setTimeout(()=>{bindObserver();queueSync();},ms));
  };
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();
  window.LifeRouteThemeAccordionV1={sync:queueSync};
})();
