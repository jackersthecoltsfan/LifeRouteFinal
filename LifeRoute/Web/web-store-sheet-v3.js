...
// Owns Search stores at the window capture phase, before the core Day card
// delegate can rerender/toggle the boundary panel. Results live in a body-level
// sheet, so route-time updates can rebuild the Day without destroying search UI.
(() => {
  ...
  const openSearch = async (button, todoID) => {
    ...
    try {
      let locations = [];
      if (typeof window.LifeRouteWebStoreDirectV2?.search === "function") {
        locations = await window.LifeRouteWebStoreDirectV2.search({ queries, nearAddresses, limitPerQuery:5 });
      } else if (typeof window.LifeRouteWebRouting?.searchStores === "function") {
        // Direct-v2 should normally be available; this keeps the sheet useful if
        // a browser blocks that helper but the older routing bridge is healthy.
        const requestID = `sheet-${token}-${Date.now()}`;
        const capture = new Promise(resolve => {
          const previous = window.lifeRouteNativeEvent;
          const wrapped = evt => {
            try { previous?.(evt); } catch (_) {}
            if (evt?.type === "storeLocations" && evt?.requestID === requestID) {
              window.lifeRouteNativeEvent = previous;
              resolve(Array.isArray(evt.locations) ? evt.locations : []);
            }
          };
          window.lifeRouteNativeEvent = wrapped;
          setTimeout(() => { if (window.lifeRouteNativeEvent === wrapped) window.lifeRouteNativeEvent = previous; resolve([]); }, 18000);
        });
        window.LifeRouteWebRouting.searchStores({ requestID, queries, nearAddresses, limitPerQuery:5 }).catch(() => {});
        locations = await capture;
      }
  ...
  // Window capture runs before the core document capture delegate. This makes
  // Search stores a true web-only surface and prevents the Day card from
  // toggling/rerendering in response to the same tap.
  ...
  ensureSheet().addEventListener("click", event => {
    if (event.target === ensureSheet()) closeSheet();
  });
