...
require(domain.count("let clientID: UUID") >= 3, "Icons, boards, and schedules carry durable client UUID ownership")
require(domain.count("var clientCode: String") >= 3, "Visual models retain the editable ABA client code for display/context")
require("func icons(for clientCode: String)" in domain, "Icon library is requested in client context")
require("func choiceBoards(for clientCode: String)" in domain, "Choice boards are requested in client context")
require("func schedules(for clientCode: String)" in domain, "Visual schedules are requested in client context")
require("clientID(forCode: code)" in domain, "Visual creation resolves client code to durable client identity")
require("throw ClientVisualSupportError.missingClient" in domain, "Visual creation requires an existing client")
