...
@main struct LivingEventTests {
    static func main() throws {
        ...
        let function=args[2]=="bat" ? "livingBatEventProbe" : "livingGustEventProbe"
        ...
        if args[2]=="bat" {
            var starts:[Float]=[]
            for cycle in 0..<30 {
                let base=Float(cycle)*32
                let origin=probe([SIMD2(base,1)])[0],start=origin.w
                starts.append(start)
                let offsets:[Float]=[-0.01,0.5,1.01,9.99,10.5,11.01]
                ...
                for boundary:Float in [0,1,10,11] {
                ...
                let edge=probe([SIMD2(base+31.9999,1),SIMD2(base+32.0001,1)])
                expect(hypot(edge[0].x-edge[2].x,edge[0].y-edge[2].y)<0.0002,"no teleport between alternating roost cycles")
            }
            let rests=zip(starts,starts.dropFirst()).map{Double($1-$0)-11}
            expect(rests.allSatisfy{10...30 ~= $0},"natural uninterrupted roost duration10-30s")
            let fraction=Double(stride(from:0,to:rows.count,by:2).filter{rows[$0].z==0}.count)/Double(times.count)
            expect(fraction>0.60 && fraction<0.75,"more runtime roosting than flying")
            expect(Set(starts.enumerated().map{Int(($0.element-Float($0.offset)*32)*100)}).count>10,"departure timing varies")
            expect(stride(from:0,to:rows.count,by:2).allSatisfy{rows[$0].x>=0.12 && rows[$0].x<=0.85 && rows[$0].y>=0.20 && rows[$0].y<=0.42},"path stays inside bounded photographed canyon region")
            ...
            report["takeoffSeconds"]=1;report["flightSeconds"]=9;report["landingSeconds"]=1
        }
        if args[2]=="gust" {
            var starts:[Float]=[]
            for cycle in 0..<30 {
