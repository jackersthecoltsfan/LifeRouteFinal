...
@main struct LivingThemeTests {
    static func main() {
        ...
        for id in ["royal", "dynamic.emeraldFlow", "scenery.rainforest.night", "scenery.ocean.day", "unknown"] {
            expect(LivingThemeScene.scene(for: id) == nil, "one scene only; other catalogs keep existing dispatch")
        }
        ...
        expect(reduced.framesPerSecond == 0 && reduced.motionAmount == 0 && reduced.atmosphereAmount == 0, "Reduce Motion keeps a static scene with no driver")
        ...
        expect(LivingSceneQuality.resolve(reduceMotion: false, lowPower: false, thermal: .critical, effectsEnabled: true) == reduced, "critical thermal is static")
        expect(LivingSceneQuality.resolve(reduceMotion: false, lowPower: false, thermal: .nominal, effectsEnabled: false) == reduced, "no-effects comparison is static")
                ...
                for attached in [true, false] {
                    for applicationActive in [true, false] {
                        for animation in [true, false] {
                            ...
                            expect(!playback.runsContinuously(attached: attached, applicationActive: applicationActive, quality: reduced), "Reduce Motion never runs the driver")
                        }
                    }
                }
            }
