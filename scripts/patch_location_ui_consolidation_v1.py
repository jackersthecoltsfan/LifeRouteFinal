...
startup_prompt = '''    // Ask for When-In-Use location once the native interface has settled. iOS itself
...
if startup_prompt in text:
    text = text.replace(startup_prompt, '''    // Location permission is user-initiated from the Home & location control.
    // Previously enabled live location resumes through live-location-v2.js while visible.
''', 1)
elif "Location permission is user-initiated from the Home & location control." not in text:
