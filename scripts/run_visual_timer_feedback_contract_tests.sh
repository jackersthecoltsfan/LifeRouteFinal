...
if [[ "${LIFEROUTE_VISUAL_TIMER_NATIVE_ONLY:-0}" == "1" ]]; then
  test "$(uname -s)" = "Darwin"
  test "${LIFEROUTE_VISUAL_TIMER_PREREQUISITE_GATE:-}" = "Q-SEM-FULL"
  python3 scripts/validate_qualification_prerequisite.py \
    --gate "$LIFEROUTE_VISUAL_TIMER_PREREQUISITE_GATE" \
  ...
  echo "Skipped host-independent Visual Timer feedback fixture: exact Q-SEM-FULL prerequisite was verified."
else
  LIFEROUTE_VISUAL_TIMER_NATIVE_ONLY=0
fi
