...
struct VisualActivityContractTests {
    ...
    private static func testThemeCenterVisibilityBridge() {
        ...
        expect(coordinator.livingEnvironmentRenderingIsActive, "Living motion remains active inside Theme Center")
        let rootInteraction = coordinator.acquireForegroundInteraction()
        expect(coordinator.livingEnvironmentRenderingIsActive, "root interaction cannot freeze Living scenery")
        let timerCoverage = coordinator.acquireAmbientSuspension()
        expect(!coordinator.livingEnvironmentRenderingIsActive, "Timer coverage still suspends Living resources")
        coordinator.releaseAmbientSuspension(timerCoverage)
        expect(coordinator.livingEnvironmentRenderingIsActive, "ending coverage resumes through foreground requests")
        coordinator.releaseAmbientSuspension(rootInteraction)
        coordinator.setThemeCenterVisible(true)
